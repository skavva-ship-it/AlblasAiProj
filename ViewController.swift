...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let pencolorbtn = dropdownbtn(title: "P", actions: [
            UIAction(title: "Black", handler: { _ in self.pencolor = .black }),
            UIAction(title: "Red", handler: { _ in self.pencolor = .red }),
            UIAction(title: "Blue", handler: { _ in self.pencolor = .blue }),
            UIAction(title: "Green", handler: { _ in self.pencolor = .green }),
            UIAction(title: "Purple", handler: { _ in self.pencolor = .purple })
        ])

        let bgcolorbtn = dropdownbtn(title: "B", actions: [
            UIAction(title: "White", handler: { _ in
                self.bgcolor = .white
                self.refreshdraw()
            }),
            UIAction(title: "Beige", handler: { _ in
                self.bgcolor = UIColor(red: 1.0, green: 0.97, blue: 0.88, alpha: 1.0)
                self.refreshdraw()
            }),
            UIAction(title: "Gray", handler: { _ in
